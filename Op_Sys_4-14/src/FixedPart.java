public class FixedPart extends Thread{
    //Partition number.
    ...
    //Constructor.
    public FixedPart(int index, int size){
        INDEX = index;
        SIZE = size;
    }
    ...
    public void run() {
        if(job != null){
            try { FixedPart.sleep(job.getExTime()); } catch (InterruptedException ex) {}
            done = true;
        }
    }
